...
public class SanfewDialogue extends DialoguePlugin {
	...
	public boolean handle(int interfaceId, int buttonId) {
		switch (stage) {
		case 0:
			if (player.getQuestRepository().getQuest(DruidicRitual.NAME).getStage(player) == 20) {
		...
		case 8:
			...
			player.getQuestRepository().getQuest(DruidicRitual.NAME).setStage(player, 20);
			stage = 9;
			break;
		case 9:
			end();
		...
		case 202:
			...
			player.getQuestRepository().getQuest(DruidicRitual.NAME).setStage(player, 99);
